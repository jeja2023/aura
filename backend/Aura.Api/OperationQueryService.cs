...
{
    internal sealed record OperationQueryResult(object Data, object Pager, bool Succeeded);
    ...
    public async Task<OperationQueryResult> GetOperationsAsync(string? keyword, int page, int pageSize, DateTimeOffset? from = null, DateTimeOffset? to = null)
    {
        if (page <= 0) page = 1;
        if (pageSize <= 0) pageSize = 20;
        if (pageSize > 100) pageSize = 100;

        var dbResult = await _auditRepository.GetOperationsAsync(keyword, page, pageSize, from, to);
        if (_pgSqlConnectionFactory.IsConfigured)
        {
            if (!dbResult.Succeeded)
            {
                return new OperationQueryResult(Array.Empty<object>(), new { page, pageSize, total = 0 }, false);
            }

            return new OperationQueryResult(dbResult.Rows, new { page, pageSize, total = dbResult.Total }, true);
        }
        ...
        return new OperationQueryResult(rows, new { page, pageSize, total }, true);
    }
}
