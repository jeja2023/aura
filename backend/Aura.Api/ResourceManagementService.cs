...
internal sealed class ResourceManagementService
{
    ...
    public ResourceManagementService(AppStore store, PgSqlStore db, string storageRoot)
    {
        _store = store;
        _db = db;
    ...
    {
        var nodes = await _db.GetCampusNodesAsync();
        if (nodes.Count > 0)
        {
    ...
    public async Task<IResult> CreateCampusAsync(CampusCreateReq req)
    {
        ...
        var dbId = await _db.InsertCampusNodeAsync(req.ParentId, req.LevelType, req.NodeName);
        if (dbId.HasValue)
        {
            await _db.InsertOperationAsync("楼栋管理员", "资源节点创建", $"节点={req.NodeName}");
    ...
    {
        var ok = await _db.UpdateCampusNodeAsync(nodeId, req.NodeName);
        if (ok)
        {
            await _db.InsertOperationAsync("楼栋管理员", "资源节点更新", $"节点ID={nodeId}");
        ...
        if (entity is null)
        {
            return Results.NotFound(new { code = 40421, msg = "节点不存在" });
        }
    ...
    {
        var ok = await _db.DeleteCampusNodeAsync(nodeId);
        if (ok)
        {
            await _db.InsertOperationAsync("楼栋管理员", "资源节点删除", $"节点ID={nodeId}");
    ...
    {
        var rows = await _db.GetFloorsAsync();
        return rows.Count > 0
    ...
    {
        var dbId = await _db.InsertFloorAsync(req.NodeId, req.FilePath, req.ScaleRatio);
        if (dbId.HasValue)
        {
            await _db.InsertOperationAsync("楼栋管理员", "楼层图创建", $"节点ID={req.NodeId}");
    ...
    public async Task<IResult> UploadFloorAsync(HttpRequest request)
    {
        ...
        await _db.InsertOperationAsync("楼栋管理员", "楼层图上传", $"文件={safeName}");
    ...
    {
        var rows = await _db.GetCamerasAsync();
        return rows.Count > 0
            ? Results.Ok(new { code = 0, msg = "查询成功", data = rows })
    ...
    {
        var dbId = await _db.InsertCameraAsync(req.FloorId, req.DeviceId, req.ChannelNo, req.PosX, req.PosY);
        if (dbId.HasValue)
        {
            await _db.InsertOperationAsync("楼栋管理员", "摄像头布点创建", $"楼层ID={req.FloorId}, 设备ID={req.DeviceId}");
    ...
    {
        var rows = await _db.GetRoisAsync();
        if (rows.Count > 0)
        {
    ...
    public async Task<IResult> SaveRoiAsync(RoiReq req)
    {
        ...
        var dbId = await _db.InsertRoiAsync(req.CameraId, req.RoomNodeId, req.VerticesJson);
