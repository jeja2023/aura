...
internal sealed class ExportApplicationService
{
    ...
    public async Task<IResult> ExportAsync(string type, string dataset, int maxRows, string? keyword = null)
    {
        ...
        {
            return Results.BadRequest(new { code = 40062, msg = "dataset 仅支持 capture/alert/judge/operation/system/user" });
        }
        ...
        {
            var dbResult = await _db.GetOperationsAsync(keyword, 1, maxRows);
            var source = (dbResult.Total > 0 || dbResult.Rows.Count > 0)
            ...
            [
                ["操作ID", "操作员", "动作", "详情", "创建时间"],
                ..source.Select(x => new[] { $"{x.OperationId}", x.OperatorName, x.Action, EscapeCell(x.Detail), x.CreatedAt.ToString("yyyy-MM-dd HH:mm:ss") })
            ];
        }
        else if (dataset == "system")
        {
            var dbResult = await _db.GetSystemLogsAsync(keyword, 1, maxRows);
            var source = (dbResult.Total > 0 || dbResult.Rows.Count > 0)
            ...
            [
                ...
                ..source.Select(x => new[] { $"{x.SystemLogId}", x.Level, x.Source, EscapeCell(x.Message), x.CreatedAt.ToString("yyyy-MM-dd HH:mm:ss") })
            ];
        }
        else
        {
            var dbUsers = await _db.GetUsersAsync();
            var source = dbUsers.Count > 0
            ...
            [
                ...
                ..ordered.Select(x => new[]
                {
                    ...
                    x.LastLoginAt?.ToString("yyyy-MM-dd HH:mm:ss") ?? ""
                })
            ];
        }
