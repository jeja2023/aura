...
- 取消 `/storage` 的匿名静态目录映射，改为经过认证、角色、权限和租户范围校验的受控文件端点。
- 证据导出只能通过带授权、有效期和下载次数控制的业务 API 获取；日志、运行时文件不再通过通用文件路由暴露。
- 楼层图片与抓拍图片仅向楼栋管理员或超级管理员开放；媒体分析和移动证据按租户及细粒度权限校验。
- 未知文件类型默认拒绝，HTML、SVG、PDF 等非安全内联类型强制下载，文件响应使用私有且禁止缓存策略。

### 数据一致性与数据库故障语义

- 数据库已配置时，用户、角色、设备、园区、抓拍、告警、研判、扩展和系统配置仓储不再静默返回空集合、`false` 或内存成功结果。
- 新增统一 `DataAccessUnavailableException`，PostgreSQL 连接与仓储异常由全局错误处理转换为标准 `503` 响应并保留追踪 ID。
- 内存回退仅允许在未配置 PostgreSQL 且明确启用开发回退时使用，生产环境不再出现“写入失败但接口成功”的假成功。
- 设备心跳写入 PostgreSQL 的 `last_seen_at`，列表缓存同步失效；新增迁移 `037_harden_capture_ingest_and_device_heartbeat.sql` 及状态索引。
- 用户停用、资料或角色变更、管理员重置密码、删除用户和显式会话撤销时，服务端会话及 Redis 鉴权缓存同步失效。

### 抓拍接入与重试可靠性

- `/api/capture/push`、`/api/capture/sdk`、`/api/capture/onvif` 的 HMAC 正文升级为设备、通道、时间戳、nonce 和原始请求体 SHA-256。
- 新增可配置时钟窗口、一次性 nonce 和 Redis 原子防重放；生产环境 Redis 不可用时默认拒绝抓拍，不降级为无保护接收。
- 重试队列由直接出队升级为 pending/processing 双队列、显式 ACK、异常恢复和原子重新入队，避免进程中断造成任务丢失。
- Redis 分布式锁使用令牌比对后删除并支持租约续期；只有处理成功且 ACK 成功后才清理暂存图片，重新入队失败不再误报成功。

### AI 生产安全

- .NET API 与 FastAPI 在生产环境强制至少 24 字符的非占位 AI API Key，并使用常量时间比较验证请求密钥。
- FastAPI 生产环境关闭 Swagger、ReDoc 和 OpenAPI 文档端点。
- `/ai/extract-file` 与离线评测数据集路径在生产环境采用默认拒绝策略，仅允许访问显式配置的绝对目录。
- Docker AI 服务明确设置 `AURA_ENV=Production`，同步文件提取和评测目录白名单配置。

### 工程治理与自动化验证

- 删除不参与构建的 `generated/` 旧源码副本、重复静态资源中间件和废弃资源服务，消除双份实现漂移。
- 新增受控存储、数据库回退语义、抓拍原始请求体摘要与 nonce 重放测试，以及 AI 生产安全测试。
- 前端引入 Playwright，在 Chromium 桌面与 Pixel 7 视口执行登录页面脚本错误、关键控件和横向溢出冒烟检查。
- GitHub Actions 安装 Chromium 并执行前端冒烟门禁；商业制品校验、迁移清单和发布门禁默认版本同步至迁移 `037`。

### 版本与升级说明

- 启用版本 `0.4.0`：同步 .NET 程序集、AI OpenAPI、镜像标签、离线镜像包、README、能力矩阵和适配器兼容声明。
- 抓拍签名契约与旧版不兼容，升级前必须同步更新设备或网关客户端；详细规范见 `docs/commercial/API迁移指南.md` 和 `docs/抓拍链路端到端测试清单.md`。
- 新增 `docs/2026-07-27-0.4.0商业产品化实施与验收记录.md`，记录自动化结果、部署前置条件和仍需现场完成的门禁。
